          ...
          script{
            ...
            withCredentials([sshUserPrivateKey(credentialsId: 'sshUser',keyFileVariable: 'identity', passphraseVariable: '',usernameVariable:'userName')]) {
              ...
              stage("Enforce with Ansible") {
                sshCommand remote: remote, sudo: true, command: 'sh -c cd /root/secops/ansible && git pull origin'
                sshCommand remote: remote, sudo: true, command: 'sh -c cd /root/secops/ansible && ansible-playbookcompliance.yaml'
              }
